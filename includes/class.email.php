...
class KPMG_Email {
	...
	// Send Notify Host Email
	public function sendNotifyHostEmail($data)
	{
		...
		if ($data['email_type'] = 'terminated')
		{
			...
			If you have any questions, please contact us via the WinterFest mailbox gtawinterfest@kpmg.ca <br /><br />
...
OJAMBO;
	...
	// Send Register Waiting List Email
	public function sendRegisterWaitingListEmail($data)
	{
		$to = $data['email_address'];
		$subject = "WinterFest 2016 waiting list";
		$details = "";
		
		// Create Details Confirmation
		// Email 19 – You are on the waiting list
		$details .= <<<OJAMBO
		Thank you for registering for WinterFest 2016. <br /><br />
		As the overall capacity for this event has been reached or the registration deadline has passed, you are on the waiting list for WinterFest2016. You will not be able to register a guest or host a group at this stage. <br /><br />
		If you have any questions, please contact us via the WinterFest mailbox gtawinterfest@kpmg.ca <br /><br />
		KPMG WinterFest Crew<br />
OJAMBO;
	...
	// Send Waiting List Register Email
	public function sendWaitingListRegisterEmail($data)
	{
		$to = $data['email_address'];
...
OJAMBO;
			...
			// Create Mime Boundry
			//$mime_boundary = "Winterfest Reserved Group-".md5(time());
	...
	// Send Reserve Group New Host Email
	public function sendReserveGroupNewHostEmail($data)
	{
		$to = "";
		$subject = "WinterFest 2016 group booking new host!";
		$details = "";
		$list_group_names = $this->generateGroupListNames($data);
		...
		foreach ($data as $key => $row)
		{
			if ($key == 0 )
			{
				// Email 15 – WinterFest Group, You are now the new host! 
				// Create Host Details Confirmation
				$details = <<<OJAMBO
				You are now the new host of your group as the original host has removed him/herself from this group.  <br /><br />
						
				Your group has the following guests. All KPMG employees in your group will also receive a confirmation email stating you are the new host for this group.  <br /><br />
				{$list_group_names}<br />

				Please keep this email for your records. If you cancel your registration, the third KPMG employee contact name on this list will automatically become the host of this group. <br /> <br />
...
OJAMBO;
			...
			{
				// Email 16 – You have a new host for your WinterFest group
				$subject = "WinterFest 2016 group booking HOST change!";
				// Create Employee Details Confirmation
				$details = <<<OJAMBO
				This email is to confirm that {$row['host_first_name']} {$row['host_last_name']} is now the new host for this group.  Should you wish to cancel your seating reservation, please contact {$row['host_first_name']} {$row['host_last_name']} and ask to be removed from the group.  <br /><br />
...
OJAMBO;
	...
	// Send Inform Employee Cancellation Request Group Removal Email
	function sendRegsitrationCancellationGroupRemovalEmail($data)
	{
		global $wpdb;
		
		$to = $data['employee_email_address'];
		$subject = "WinterFest registration group change";
		$message = "";
		
		// Email 17 – You have been successfully removed from a group
		$message .= <<<OJAMBO
		Hello {$data['employee_first_name']} {$data['employee_last_name']},
		This is to confirm that you have been <b style="font-weight: bold;"<u style="text-decoration: underline;">successfully</u></b>removed from your group. <br /><br />
...
OJAMBO;
	...
	// Send Inform Employee Cancellation Request Email
	function sendRegistrationCancellationEmail($data)
	{
		global $wpdb;
		
		$to = $data['employee_email_address'];
		$subject = "WinterFest registration CANCELLATION confirmation";
		$message = "";
...
OJAMBO;
	...
	// Send CSV Email
	function sendCSVEmail($csvFile, $csvName, $csvSubject, $email)
	{
		$to = $email;
		$subject = !empty($csvSubject) ? $csvSubject : "Winterfest CSV Report";
		$filename = !empty($csvName) ? $csvName : "report.csv";
